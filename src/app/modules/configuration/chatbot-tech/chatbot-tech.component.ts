...
export class ChatbotTechComponent extends DestroyObservable implements OnInit {
  ...
  resetData() {
    ...
    dialogRef
      ...
      .pipe(filter((r) => !!r))
      .subscribe(() => {
        this.adminService.resetData().subscribe(() => {
          this._toast.success(
  ...
  saveTechConfig() {
    this._configService
      .saveConfig(Utils.getDirtyValues(this.techForm))
      .subscribe(() => {
        this._toastr.success(
          'La configuration technique du chatbot a bien été modifiée',
        );
  ...
  private _initForms() {
    this.techForm = this._fb.group({
      ...
      apiKey: [
        { value: this.chatbotConfig.apiKey, disabled: true },
        [Validators.required],
      ],
      delayBetweenMessages: [
        this.chatbotConfig.delayBetweenMessages,
